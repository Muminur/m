...
pub enum UndoOperation {
    UpdateText { segment_id: String, old_text: String, new_text: String },
    DeleteSegment { segment_id: String, transcript_id: String, index_num: i64, start_ms: i64, end_ms: i64, text: String, speaker_id: Option<String>, confidence: Option<f64> },
    MergeSegments { kept_id: String, removed_id: String, old_kept_text: String, old_removed_text: String, merged_text: String, old_kept_end_ms: i64, new_kept_end_ms: i64 },
    SplitSegment { original_id: String, new_id: String, old_text: String, first_text: String, second_text: String, split_ms: i64 },
}
...
mod tests {
    ...
    fn test_push_and_pop_undo() {
        ...
        mgr.push(UndoOperation::UpdateText {
            segment_id: "s1".into(), old_text: "old".into(), new_text: "new".into(),
        }).unwrap();
        assert!(mgr.can_undo());
    ...
    fn test_redo_cleared_on_new_push() {
        let mgr = UndoManager::new();
        mgr.push(UndoOperation::UpdateText {
            segment_id: "s1".into(), old_text: "a".into(), new_text: "b".into(),
        }).unwrap();
        ...
        mgr.push(UndoOperation::UpdateText {
            segment_id: "s2".into(), old_text: "c".into(), new_text: "d".into(),
        }).unwrap();
        assert!(!mgr.can_redo());
    }
    ...
    fn test_max_stack_size() {
        ...
        for i in 0..60 {
            mgr.push(UndoOperation::UpdateText {
                segment_id: format!("s{}", i), old_text: "old".into(), new_text: "new".into(),
            }).unwrap();
        }
